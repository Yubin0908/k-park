...
INSERT INTO SUGGEST (SNO, ID, STITLE, STEXT, SRDATE, SGROUP, SSTEP, SINDENT, SIP)
  VALUES (SUGGEST_SQ.NEXTVAL, 'ccc', '예약을 쉽게', '하는방법', SYSDATE, SUGGEST_SQ.CURRVAL, 0,0, '192.168.0.254');--글1
INSERT INTO SUGGEST (SNO, ID, STITLE, STEXT, SRDATE, SGROUP, SSTEP, SINDENT, SIP)
...
SELECT *
    FROM (SELECT ROWNUM RN, A.* FROM (SELECT SNO, NVL((SELECT NAME FROM MEMBER WHERE SUGGEST.ID=ID), '관리자') WRITER, STITLE, SHIT, SRDATE 
        FROM SUGGEST ORDER BY SRDATE DESC) A)
