...
SELECT 
	    ...
	    D.PARKNAME AS EXPLORE_PARKNAME, d.EXPLORENAME
			...
			LEFT JOIN SHELTER C ON A.SHELTERNO = C.SHELTERNO
			LEFT JOIN EXPLORE D ON A.EXPLORENO = D.EXPLORENO
			WHERE (B.PARKNAME = '설악산' OR C.PARKNAME = '설악산' OR D.PARKNAME = '설악산') AND TO_CHAR(RESDATE, 'YYYY-MM-DD') = '2023-11-25';
...
-- [관리자] 예약일자 등록처리[지정일]
-- CAMP [type = camp]
INSERT INTO RESERVED (RESNO, CAMPNO, RESDATE, REM) VALUES (RESERVED_SQ.NEXTVAL, 100, '2023-12-01', 20);
-- SHELTER [type = shelter]
INSERT INTO RESERVED (RESNO, SHELTERNO, RESDATE, REM) VALUES (RESERVED_SQ.NEXTVAL, 200, '2023-12-01', 20);
-- [관리자] 날짜 등록 여부 조회
SELECT COUNT(*) FROM RESERVED WHERE CAMPNO = 108 AND TO_CHAR(RESDATE, 'YYYY-MM-DD') = '2023-11-23';
SELECT COUNT(*) FROM RESERVED WHERE SHELTERNO = 200 AND RESDATE = '2023-11-30';

DELETE FROM RESERVED WHERE CAMPNO = 108 AND TO_CHAR(RESDATE, 'YYYY-MM-DD') = '2023-12-01';
